"""Top-level package for Python MagnetDB."""
...
try:
    __version__ = version("python-magnetdb")
...
def check_magnetgeo_compatibility():
    ...
    try:
        ...
        if not (min_ver <= current < max_ver):
            raise RuntimeError(
                f"python_magnetdb requires python_magnetgeo >={MIN_MAGNETGEO_VERSION},<{MAX_MAGNETGEO_VERSION} "
